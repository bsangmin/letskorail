...
코레일 예약 시스템은 자리배정 -> 예약 -> 결제 -> 예약확정 순으로 흘러갑니다.

이 프로젝트는 자리배정 -> 예약 단계만 지원합니다.

이후는 직접 코레일 앱에서 해야됩니다.

예약 -> 결제 단계로 넘어갈 때 디바이스의 고유번호(uuid)를 요구하는데 고유번호를 생성하여 결제 할 수 있지만 앱에서 확인하면 다른 디바이스에서 결제한 승차권이라고 표시되어 상황에 따라 부정 승차권으로 간주 됩니다.

예약이 후 20분내로 결제를 완료해야 확정이되고 이후엔 자동으로 취소됩니다.
